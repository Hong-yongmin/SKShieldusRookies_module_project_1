...
def recommend_places(region, theme, age="20대", group="1명", sex="남성"):
    ...
    return json.dumps({
        ...
        "tourist_attractions": tour_df[["name", "category", "address", "mapx", "mapy"]].to_dict(orient="records") if not tour_df.empty else [],
        "restaurants": food_df[["name", "category", "address", "mapx", "mapy"]].to_dict(orient="records") if not food_df.empty else []
    }, ensure_ascii=False)
